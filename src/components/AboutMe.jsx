...
const AboutMe = () => {
  return (
    ...
    >
      ...
      <div className="relative max-w-5xl mx-auto text-center space-y-6 lg:space-y-8 px-4">
        {/* Animated Heading */}
        <motion.h2
          initial={{ opacity: 0, y: 40 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.8 }}
          className="text-3xl sm:text-4xl md:text-5xl font-bold text-indigo-400"
        >
          About Me
        </motion.h2>
        ...
        >
          <img
            src={chandImage}
            alt="Chandan Verma"
            className="w-48 h-56 sm:w-56 sm:h-68 md:w-60 md:h-72 object-cover shadow-2xl rounded-lg"
          />
        </motion.div>
        ...
        <motion.div
          ...
          className="flex flex-wrap justify-center gap-2 sm:gap-3 md:gap-4 mt-6 lg:mt-8"
        >
          {[
            "HTML • CSS • JavaScript",
            ...
            <motion.span
              ...
              className="px-3 sm:px-4 md:px-6 py-1.5 sm:py-2 rounded-full bg-indigo-500/20 border border-indigo-400 text-indigo-300 cursor-default backdrop-blur-sm hover:bg-indigo-500/30 transition-all duration-300 text-xs sm:text-sm md:text-base"
            >
              {skill}
            </motion.span>
          ))}
